...
export default function CreateUserModal({ onClose, onSuccess }) {
  ...
  const fetchBlocks = async () => {
    try {
      ...
      const res = await fetch(`${API}/api/admin/blocks`, { headers: { 'Authorization': `Bearer ${token}` } })
      if (res.ok) { const data = await res.json(); setBlocks(data) }
        ...
        {step === 1 ? (
          <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  { val: 'super_admin', label: 'Admin', icon: '🛡️' },
                ].map(r => (
                  <button key={r.val} onClick={() => setForm({ ...form, role: r.val })}
                    className={`flex flex-col items-center gap-1 p-3 rounded-xl border-2 transition text-sm font-semibold ${form.role === r.val ? 'bg-purple-600/30 border-purple-500 text-white' : 'bg-slate-800/50 border-slate-700 text-slate-400 hover:border-slate-600'}`}>
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPassword ? 'text' : 'password'} value={form.password} onChange={e => handlePasswordChange(e.target.value)}
                ...
                <button onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-white text-sm">
                  {showPassword ? '🙈' : '👁️'}
                </button>
              </div>
              {form.password && (
                <div className="mt-2">
                  <div className="flex gap-1 mb-1">
                    {[1,2,3,4,5].map(i => (
                      <div key={i} className={`h-1 flex-1 rounded-full transition-all ${i <= passwordStrength ? strengthColor[passwordStrength] : 'bg-slate-700'}`} />
                    ))}
                  </div>
            ...
            {/* Block assignment */}
            <div>
              <label className="block text-slate-400 text-xs font-semibold uppercase tracking-wider mb-2">Block / Section</label>
              <select value={form.block_id} onChange={e => setForm({ ...form, block_id: e.target.value })}
                ...
                {blocks.map(b => <option key={b.id} value={b.id}>{b.section_code}{b.semester ? ` · ${b.semester}` : ''}</option>)}
              </select>
            </div>
            ...
            {form.role === 'student' && (
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-400 text-xs font-semibold uppercase tracking-wider mb-2">Student ID</label>
                  <input type="text" value={form.student_id_number} onChange={e => setForm({ ...form, student_id_number: e.target.value })}
          ...
          <div className="p-6 text-center space-y-4">
            ...
            <div className="bg-slate-800/80 rounded-xl p-4 text-left space-y-2 border border-slate-700">
              ...
              <Row label="Role" value={<span className="capitalize px-2 py-0.5 bg-purple-600/30 text-purple-300 rounded text-xs">{createdUser?.role}</span>} />
