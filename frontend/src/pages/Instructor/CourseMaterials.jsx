...
export default function CourseMaterials({ classId }) {
    ...
    const fetchSections = async () => {
      try {
        ...
        const res = await fetch(
          `http://localhost:5000/api/instructor/blocks-by-subject?subject_id=${formData.subject_id}`,
          { headers: { 'Authorization': `Bearer ${token}` } }
        )
        if (res.ok) {
          const data = await res.json()
          setSections(data)
          // Clear selected section if it no longer belongs to this subject
          setFormData(prev => ({
            ...prev,
            section_id: prev.section_id && data.some(s => s.id === parseInt(prev.section_id))
              ? prev.section_id
              : ''
          }))
        }
  ...
  const handlePublish = async (lessonId) => {
    try {
      ...
      if (res.ok) {
        ...
        const refreshRes = await fetch('http://localhost:5000/api/lessons', {
          headers: { 'Authorization': `Bearer ${token}` }
        })
        if (refreshRes.ok) {
  ...
  return (
    <div className="space-y-6">
      ...
      {error && (
        <div className="p-4 bg-red-900/30 border border-red-600/40 rounded-xl text-red-300">
          ❌ {error}
        </div>
      )}
      {success && (
        <div className="p-4 bg-green-900/30 border border-green-600/40 rounded-xl text-green-300">
          ✅ {success}
        </div>
      )}
      ...
      {showCreateForm ? (
        <form onSubmit={handleSubmit} className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white focus:border-green-500 outline-none transition"
              />
            </div>
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white outline-none focus:border-green-500 transition"
              />
              <p className="text-slate-500 text-xs mt-1">Organise lessons by week</p>
            </div>
          </div>
          ...
          <div>
            ...
            {loadingSubjects ? (
              ...
              <select
                name="subject_id" value={formData.subject_id}
                onChange={handleChange} required
          ...
          {/* ✅ FIX: Section / target audience selector
               The dropdown now shows SubjectSection objects (section_no + semester).
               Selecting one targets that section's block. Leaving blank = all sections. */}
          <div>
            <label className="block text-slate-400 text-sm mb-2">🏫 Target Section (Class Code)</label>
            {loadingSections ? (
              <div className="h-12 bg-slate-800 rounded-xl animate-pulse"></div>
            ) : (
              <select
            ...
            {formData.subject_id && sections.length > 0 && !loadingSections && (
              <p className="text-slate-500 text-xs mt-1">
                🌐 "All My Sections" = visible to:{' '}
                {sections.map(s => `Section ${s.section_no}`).join(', ')}
              </p>
            )}

            {formData.subject_id && sections.length === 0 && !loadingSections && (
              <p className="text-yellow-500 text-xs mt-1">
                ⚠️ You are not assigned to teach this subject in any section
              </p>
            )}
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white focus:border-green-500 outline-none resize-none transition"
            />
          </div>
                ...
                {formData.files.map((file, index) => (
                  <div key={index} className="flex items-center justify-between bg-slate-800 rounded-lg px-4 py-2 border border-slate-700">
                    ...
                    <button
                      ...
                      className="text-red-400 hover:text-red-300 text-sm px-2 py-1 hover:bg-red-900/30 rounded transition"
                    >
                      ✕ Remove
                    </button>
                  </div>
          ...
          <div className="flex items-center gap-3 p-4 bg-slate-800/50 rounded-xl border border-slate-700">
            <input
              ...
              onChange={handleChange}
              className="w-5 h-5 rounded border-slate-600 text-green-600 focus:ring-green-500 cursor-pointer"
            />
            <div>
          ...
          <div className="flex gap-4 pt-4 border-t border-slate-800">
            ...
            <button
              type="submit"
              disabled={isSubmitting || !formData.subject_id}
              className="flex-1 py-3 bg-green-600 hover:bg-green-500 disabled:bg-slate-700 disabled:cursor-not-allowed rounded-xl text-white font-bold transition shadow-lg shadow-green-600/20 flex items-center justify-center gap-2"
            >
              {isSubmitting
        ...
        <div className="text-center py-12 bg-slate-900 border border-slate-800 rounded-2xl">
          ...
          <p className="text-slate-400 text-lg mb-2">
            {classId ? 'No modules for this class yet' : 'No lessons created yet'}
          </p>
          <p className="text-slate-500 mb-6">Start by creating your first learning module above.</p>
          <button
            onClick={() => setShowCreateForm(true)}
            className="px-6 py-3 bg-green-600 hover:bg-green-500 text-white rounded-xl font-bold transition shadow-lg hover:scale-105 active:scale-95"
          >
            Create First Module
          </button>
        </div>
        ...
        <div className="space-y-4">
          ...
          {lessons
            ...
            .map(lesson => (
              <div
                key={lesson.id}
                className={`bg-slate-900 border rounded-xl p-4 hover:border-slate-700 transition cursor-pointer ${
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex gap-2">
                    {!lesson.is_published && (
                      <button
                        onClick={(e) => { e.stopPropagation(); handlePublish(lesson.id) }}
