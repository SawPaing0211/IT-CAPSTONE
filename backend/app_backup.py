...
# ✅ NEW: Gamified Registration Endpoint
@app.route('/api/register', methods=['POST'])
def register():
    data = request.get_json()
    name = data.get('name')  # Adventurer Name
    ...
    # Create new user (default: student role, level 1, 0 XP, 0 streak)
    cursor.execute("""
        INSERT INTO users (name, email, password, role, xp, level, streak)
        VALUES (%s, %s, %s, 'student', 0, 1, 0)
    """, (name, email, password))
